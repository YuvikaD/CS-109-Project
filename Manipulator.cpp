		...
		if (Rule_map[leftHandSide]->get_logop() == "OR"){	// OR rule
			...
			// AND ----------- AND ----------- AND ------------- AND --------------- AND
						...
						for (auto it2 = it->begin(); it2 != it->end(); ++it2){
							...
							//} else {
								...
								if(!varNeeded && MatchVar){ // if the string wasn't found
									...
									string useless = (*it2);
									int loopcount = 0;
									int groupSize = Rule_map[leftHandSide]->RuleVector[0][rVecIndex].size()-1;
									for (auto col = Rule_map[infRules[0]]->Results.begin(); col != Rule_map[infRules[0]]->Results.end(); ++col){
										for (auto row = col->begin(); row != col->end(); ++row){
											if(useless == (*row)){ // if we find this string that we don't need
												for(int offset = (loopcount-(loopcount%groupSize)); offset < (loopcount-(loopcount%groupSize) + groupSize); offset++){
													cout << "Erasing " << Rule_map[infRules[0]]->Results[rowLoop][(loopcount-(loopcount%groupSize))]<<endl;
													Rule_map[infRules[0]]->Results[rowLoop].erase(Rule_map[infRules[0]]->Results[rowLoop].begin()+(loopcount-(loopcount%groupSize)));
												}
											}
											loopcount++;
				...
				*/
				...
				for (auto it = Rule_map[infRules[0]]->Results.begin(); it != Rule_map[infRules[0]]->Results.end(); ++it){
					...
					for(auto it1 = Rule_map[infRules[0]]->Results.begin() + 1; it1 != Rule_map[infRules[0]]->Results.end(); ++it1){
						for (auto it2 = it->begin(); it2 != it->end(); ++it2){
							...
							for (auto it3 = it1->begin(); it3 != it1->end(); ++it3){
								//cout<<"it3++"<<endl;
								//if(distance(it->begin(), it2) == distance(it1->begin(), it3)){cout<<"nothing"<<endl;}
								//else 
								...
								if((*it2) == (*it3)) {
									...
									for (auto itt = it->begin(); itt != it->end(); ++itt){
										//if(t!=(*itt)[1]){groupSize++;}
										cout << ((*itt)[1]) << endl;
										//wtf.push_back(((*itt)[1]));
										char tt = ((*itt)[1]);
										//if(t!=tt){groupSize++;}
										//else{break;}
										//if(distance(it->begin(), it2) == distance(it->begin(), itt)){}
											//else{ 
											while(t != (*itt)[1]){
												//cout <<"itt: "<<((*itt)[1])<<endl;
												itt++;
				...
				Rule_map[leftHandSide]->dollarVarsVec
				dollar VarsVec $X $Y $X $Z $Z $Y
				
				
				Rule_map[infRules[0]]->Results
				
				for( auto iter = VARVECTOR.begin(); iter != VARVECTOR.end(); ++iter){
					cout << *iter;
				}
				Results[0] VARVECTOR: $XAllen $ZMargret $XDean $ZLily $XCarl $ZMarcie
				Results[1] VARVECTOR: $ZMargret $YBob $ZMarcie $YSarah $ZMarcie $YRyan
				
				Father: $XAllen $ZMargret $XDean $ZLily $XCarl $ZMarcie
				Mother: $ZMargret $YBob $ZMarcie $YSarah $ZMarcie $YRyan
				
				The GrandFather rule vector itself
				Vector of vectors
				0 GrandFather $X $Y
				1 Father $X $Z
				2 Parent $Z $Y
				////////////////////////////do later: look for unique variable facts, return with false if one has no results
				
				for each $X, $Z, and $Y:
					If in all results vectors with the same variable, 
						the string exists
							dont delete it
						else
							go through all the vectors and delete each group of $ Variables that contains that string
				// this narrows down the results to only variables that are the correct results
				
				new vectors: (dean and lily got deleted)
				Results[0] VARVECTOR: $XAllen$ZMargret$XCarl$ZMarcie
				Results[1] VARVECTOR: $ZMargret$YBob$ZMarcie$YSarah$ZMarcie$YRyan
