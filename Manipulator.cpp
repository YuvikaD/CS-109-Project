...
void Manipulator::printAnd(vector<vector<string>> printResults, vector<string> tVar, vector<vector<string>> Results){
	//int result = 0;
	cout << "printAND" << endl;
				//int index = 0;
	for(int r = 0; r < Results.size();r++){
		int groupSize = 1;
		char t = Results[r][0][1];
		int itt = 1;
		while(t != Results[r][itt][1]){
			//cout << "looking at "<<Results[r][itt][1]<<endl;
			itt++;
			groupSize++;
		}
		cout << "groupSize: "<<groupSize<<endl;
			
		for(int c = 0; c <= Results[r].size()-groupSize;c+=groupSize){
			...
			for(string s : tVar){
				...
				for (int count = 0; count < groupSize; count++){
					...
					//cout << (bool)(c+count) < Results[r].size()<<endl;
					//cout << "results["<<r<<"]["<<c+count<<"]: "<<Results[r][c+count]<<endl;
					//cout << "r size: "<< Results[r].size()<<endl;*/
					//cout <<"s[1]: " << s[1]<<endl;
					if((c+count) <= Results[r].size()){
						if(Results[r][c+count][1] == s[1]){
							cout << " "<< s[1] << ": ";
							for (int b = 2; b < Results[r][c+count].length(); b++){
								cout<<Results[r][c+count][b];
							}
					...
					//printResults[0].push_back(Results[r][c+count]);
					//scout<< "pushed: "<< Results[r][c+count]<< endl;
				} cout << endl;
				
				
			}
			//cout<< s[0] << Results[r][c]<<" "<<endl;
		}
